...
process.env.STATE_DEMO_PASSWORD = "estado-password-muy-seguro";
process.env.FACILITY_DEMO_USERNAME = "elepem-prueba";
process.env.FACILITY_DEMO_PASSWORD = "elepem-password-diferente";
process.env.FACILITY_DEMO_IDS = "DEMO-ELEPEM-001,DEMO-ELEPEM-003";

test("las credenciales demo están separadas por rol", () => {
  assert.equal(institutionalIdentityForCredentials("state", "estado-prueba", "estado-password-muy-seguro")?.role, "state");
  assert.equal(institutionalIdentityForCredentials("facility", "elepem-prueba", "elepem-password-diferente")?.role, "facility");
  assert.equal(institutionalIdentityForCredentials("state", "elepem-prueba", "elepem-password-diferente"), null);
  assert.equal(institutionalIdentityForCredentials("facility", "estado-prueba", "estado-password-muy-seguro"), null);
});
...
test("la sesión firmada conserva rol, identidad, vencimiento y asignaciones", () => {
  const now = Date.parse("2026-08-10T12:00:00Z");
  const identity = institutionalIdentityForCredentials("facility", "elepem-prueba", "elepem-password-diferente");
