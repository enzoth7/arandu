...
export function RepresentationRequestForm({ facilities }: { facilities: FacilityOption[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { await postJson("/api/institutional/representation-claims", { facilityId: String(data.get("facilityId")).startsWith("DEMO-") ? String(data.get("facilityId")) : Number(data.get("facilityId")) }); setState({ kind: "success", message: "Solicitud recibida. Administración comprobará la autorización." }); router.refresh(); }
    catch (error) { setState({ kind: "error", message: error instanceof Error ? error.message : "No se pudo enviar." }); }
  }
  return <form className="workflowForm" onSubmit={submit}>
