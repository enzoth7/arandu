...
export function VisitDashboard({ mode }: { mode: "visitor" | "facility" }) {
  ...
  return <div className="visitDashboard">
    ...
    {error && <p className="reportFieldError" role="alert">{error}</p>}
    {loading ? <p role="status">Cargando visitas…</p> : visits.length === 0 ? <div className="visitEmpty"><CalendarDaysIcon /><h2>No tenés visitas programadas</h2><p>{mode === "visitor" ? "Para solicitar una visita, buscá el residencial, entrá a su ficha pública y hacé clic en 'Agendar visita'." : "Las solicitudes de visita que reciba tu ELEPEM aparecerán en esta bandeja."}</p>{mode === "visitor" && <Link className="reportContinue" href="/">Buscar ELEPEM</Link>}</div> : <div className="visitCards">{visits.map((visit) => {
      ...
      return <article className="visitCard" key={visit.id}>
        <header><div><h2>{visit.facilityName}</h2><small>{visit.facilityLocality} · {visit.facilityDepartment}</small></div><span className={`visitStatus visitStatus-${visit.status}`}>{VISIT_STATUS_LABELS[visit.status]}</span></header>
