...
export async function GET(request: NextRequest, context: { params: Promise<{ attachmentId: string }> }) {
  const auth = institutionalSessionOrError(request);
  if (!auth.session) return auth.response;
  const { attachmentId } = await context.params;
  if (!UUID.test(attachmentId)) return NextResponse.json({ error: "Adjunto inválido." }, { status: 400 });
  try {
    const rows = await querySupabaseDatabase<{ object_path: string; file_name: string; mime_type: string; demo_facility_id: string | null }>(
      `SELECT attachment.object_path, attachment.file_name, attachment.mime_type, report.demo_facility_id
       ...
       WHERE attachment.id = $1 AND report.is_demo = true LIMIT 1`, [attachmentId],
    );
    const attachment = rows[0];
    if (!attachment || (auth.session.role === "facility" && (!attachment.demo_facility_id || !auth.session.facilityIds.includes(attachment.demo_facility_id)))) {
