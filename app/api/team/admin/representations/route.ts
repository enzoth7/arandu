...
export async function POST(request: NextRequest) {
  ...
  if (!UUID.test(userId) || (!isDemo && !Number.isSafeInteger(parsedId)) || !["approve", "reject", "suspend", "revoke"].includes(action)) return NextResponse.json({ error: "Decisión inválida." }, { status: 400 });
