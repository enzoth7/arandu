...
export function parseFacilityChangeSubmission(value) {
  ...
  const facilityId = typeof value.facilityId === "string" && value.facilityId.startsWith("DEMO-") ? value.facilityId : Number(value.facilityId);
  const evidenceNote = intakeText(value.evidenceNote, 2_000);
  const reasonNote = intakeText(value.reasonNote, 2_000);
  ...
  const listRules = { phones: { maximum: 20, pattern: /^[+()0-9\s.-]{6,40}$/ }, emails: { maximum: 20, pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ }, priceIncludes: { maximum: 50, pattern: /^.+$/ } };
  ...
  const photosToDelete = Array.isArray(value.photosToDelete) ? value.photosToDelete.filter(item => typeof item === 'string').map(item => intakeText(item, 1000)).filter(Boolean) : [];
  ...
  if (Object.keys(changes).length === 0 && photoCount === 0 && photosToDelete.length === 0 && !value.removeCurrentPhotos) return null;
  return {
    facilityId,
    payload: {
      version: 4,
      ...
      removeCurrentPhotos: value.removeCurrentPhotos === true,
      photosToDelete,
      photoCount,
