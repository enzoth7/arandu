...
const FUTURE_AUTHORIZATION_KEYS = new Set(["publicName", "shareContactWithFacility"]);
...
export function parseExperienceSubmission(value) {
  ...
  if (typeof publicName !== "boolean" || typeof shareContactWithFacility !== "boolean") return null;
  if (privacyMode !== "registered_identity" && (publicName || shareContactWithFacility)) return null;
  if (publicName && !contact?.name) return null;
  if (shareContactWithFacility && !contact) return null;

  const requestedDestination = value.requestedDestination;
  const destinationAllowed = privacyMode === "anonymous"
    ? requestedDestination === "private_review" || requestedDestination === "consider_anonymized"
    : requestedDestination === "private_review";
  if (!destinationAllowed || typeof value.publicationConsent !== "boolean") return null;
  const publicationConsent = privacyMode === "anonymous" && requestedDestination === "consider_anonymized";
  ...
  return {
    payload: {
      ...
      publicationConsent,
      privacyMode,
      futureAuthorizations: { publicName, shareContactWithFacility },
      consent: true,
