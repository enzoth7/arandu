...
import { FACILITY_ATTRIBUTE_FILTER_GROUPS } from "./facility-filter-options.mjs";
...
export function filterFacilities(facilities, criteria = {}, haystackFor) {
  ...
  return facilities.filter((facility) => {
    ...
    if (qualityRating === "unrated" && facility.qualityRating) return false;
    if (qualityRating && qualityRating !== "unrated" && facility.qualityRating !== qualityRating) return false;
    for (const group of FACILITY_ATTRIBUTE_FILTER_GROUPS) {
      const selected = Array.isArray(attributeFilters[group.key]) ? attributeFilters[group.key] : [];
      if (!selected.length) continue;
      const available = facility[group.key];
      if (!Array.isArray(available) || !selected.some((value) => available.includes(value))) return false;
    }
