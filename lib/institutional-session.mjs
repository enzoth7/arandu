...
export const INSTITUTIONAL_SESSION_COOKIE = "arandu-institutional-session";
...
function signature(payload) {
  return createHmac("sha256", sessionSecret())
    .update(`arandu-institutional-session:v1:${payload}`)
    .digest("base64url");
}
...
export function institutionalIdentityForCredentials(role, username, password) {
  const config = role === "state"
    ...
    : role === "facility"
      ...
      : null;
  const minimumPasswordLength = process.env.DEMO_MODE === "true" ? 1 : 12;
  if (!config || config.expectedUsername.length < 3 || config.expectedPassword.length < minimumPasswordLength) return null;
